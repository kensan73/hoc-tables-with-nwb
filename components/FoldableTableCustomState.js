...
const Ftable = FoldableHoc(selectTableHOC(ReactTable));
...
export default class FoldableTableCustomState extends React.Component {
    ...
    toggleSelection = (key, shift, row) => {
        ...
        this.setState(p => { return { selected: newSelected } });
    };
    ...
    toggleAll = () => {
        ...
        if (selectedAll)
            this.setState(p => { return { selectedAll: false, seleted: {} } });
        else {
            const data = this.getData();

            let newSelected = {};
            data.forEach(d => newSelected[d.id] = true);
            this.setState(p => { return { selectedAll: true, seleted: newSelected } });
        }
    }
